...
if ( ! class_exists( 'Teenglow_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Teenglow_Welcome_Page {
		private static $instance;

		/**
		 * @return Teenglow_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_teenglow_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_teenglow_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_teenglow_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'teenglow_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			add_theme_page(
				esc_html__( 'About', 'teenglow' ),
				esc_html__( 'About', 'teenglow' ),
				current_user_can( 'edit_theme_options' ),
				'teenglow_welcome_page',
				array( $this, 'welcome_page_content' )
			);

			remove_submenu_page( 'themes.php', 'teenglow_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( TEENGLOW_ROOT_DIR . '/screenshot.png' ) ? TEENGLOW_ROOT . '/screenshot.png' : TEENGLOW_ROOT . '/screenshot.jpg';

			teenglow_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_teenglow_welcome_page' === $hook ) {
				wp_enqueue_style( 'teenglow-welcome-page-style', TEENGLOW_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Teenglow_Welcome_Page::get_instance();
}
