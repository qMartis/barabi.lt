...
if ( ! function_exists( 'barabi_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function barabi_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'BarabiCore_Breadcrumbs_Title';

		return $layouts;
	}

	add_filter( 'barabi_core_filter_register_title_layouts', 'barabi_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'barabi_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function barabi_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'barabi-core' );

		return $layouts;
	}

	add_filter( 'barabi_core_filter_title_layout_options', 'barabi_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'barabi_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function barabi_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters(
			'barabi_core_filter_breadcrumbs_label',
			array(
				'home'        => esc_html__( 'Home', 'barabi-core' ),
				'tag'         => esc_html__( 'Posts tagged "%s"', 'barabi-core' ),
				'author'      => esc_html__( 'Posted by %s', 'barabi-core' ),
				'search'      => esc_html__( 'Search results for "%s"', 'barabi-core' ),
				'404'         => esc_html__( '404 - Page not found', 'barabi-core' ),
				'query_paged' => esc_html__( '(Page %s)', 'barabi-core' ),
			)
		);

		// Breadcrumbs variables
		$settings = apply_filters(
			'barabi_core_filter_breadcrumbs_settings',
			array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'barabi_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'barabi_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}
