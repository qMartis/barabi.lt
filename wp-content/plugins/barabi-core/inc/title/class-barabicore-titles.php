...
class BarabiCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'barabi_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'barabi_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return BarabiCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( BARABI_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( BARABI_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = barabi_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'barabi_core_filter_register_title_layouts', array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'barabi_filter_title_template' : 'barabi_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = barabi_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = barabi_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = barabi_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$in_grid            = barabi_core_get_post_value_through_levels( 'qodef_set_page_title_area_in_grid' );
	...
	function add_inline_styles( $style ) {
		...
		$height                   = barabi_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = barabi_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding            = apply_filters( 'barabi_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'barabi_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = barabi_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = barabi_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = barabi_core_get_page_title_image_params();
		$title_additional_padding = barabi_core_get_post_value_through_levels( 'qodef_page_title_additional_bottom_offset' );
		$title_side_padding       = barabi_core_get_post_value_through_levels( 'qodef_page_title_side_padding' );
		$title_in_grid            = barabi_core_get_post_value_through_levels( 'qodef_set_page_title_area_in_grid' );
		...
		$title_color = barabi_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
BarabiCore_Titles::get_instance();
