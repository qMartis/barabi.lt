...
if ( ! class_exists( 'BarabiCore_Dashboard' ) ) {
	class BarabiCore_Dashboard {
		private static $instance;

		private $sub_pages      = array();
		private $validation_url = 'https://api.qodeinteractive.com/purchase-code-validation.php';
		public $licence_field   = 'barabi_purchase_info';
		public $import_field    = 'barabi_import_params';
		...
		public function __construct() {
			...
			add_action( 'barabi_core_action_on_deactivation', array( &$this, 'remove_redirect' ) );

			if ( 'yes' === get_option( 'barabi_core_activated_first_time' ) ) {
				add_action( 'barabi_core_action_plugin_loaded', array( &$this, 'page_welcome_redirect' ) );
			}
		...
		public function set_sub_pages( BarabiCore_Dashboard_Sub_Page $sub_page ) {
		...
		function load_files() {
			include_once BARABI_CORE_INC_PATH . '/core-dashboard/rest/include.php';
			include_once BARABI_CORE_INC_PATH . '/core-dashboard/class-barabicore-dashboard-registration.php';
			include_once BARABI_CORE_INC_PATH . '/core-dashboard/class-barabicore-dashboard-theme-validation.php';
			include_once BARABI_CORE_INC_PATH . '/core-dashboard/sub-pages/class-barabicore-dashboard-sub-page.php';

			foreach ( glob( BARABI_CORE_INC_PATH . '/core-dashboard/sub-pages/*/include.php' ) as $subpages ) {
		...
		function dashboard_add_page() {
			...
			$page = add_menu_page(
				esc_html__( 'Barabi Dashboard', 'barabi-core' ),
				esc_html__( 'Barabi Dashboard', 'barabi-core' ),
				'administrator',
				'barabi_core_dashboard',
				array( &$this, 'load_dashboard_template' ),
				BARABI_CORE_INC_URL_PATH . '/core-dashboard/assets/img/admin-logo-icon.png',
				998
			);
		...
		function load_dashboard_template() {
			...
			barabi_core_template_part( 'core-dashboard', 'templates/core-dashboard', '', $params );
		}

		public function register_sub_pages() {
			$sub_pages = apply_filters( 'barabi_core_filter_add_welcome_sub_page', array() );
		...
		function enqueue_styles() {
			...
			wp_enqueue_style( 'barabi-core-dashboard-style', BARABI_CORE_INC_URL_PATH . '/core-dashboard/assets/css/core-dashboard.min.css' );
		}
		...
		function enqueue_scripts() {
			...
			wp_enqueue_script( 'barabi-core-dashboard-script', BARABI_CORE_INC_URL_PATH . '/core-dashboard/assets/js/modules/core-dashboard.js', array(), false, true );
			$global_variables = apply_filters( 'barabi_core_filter_dashboard_js_global_variables', array() );

			wp_localize_script(
				'barabi-core-dashboard-script',
		...
		function remove_redirect() {
			delete_transient( 'barabi_core_welcome_page_redirect' );
		}

		function page_welcome_redirect() {
			$redirect = get_transient( 'barabi_core_welcome_page_redirect' );

			if ( empty( $redirect ) ) {
				set_transient( 'barabi_core_welcome_page_redirect', 1, 31536000 );

				wp_safe_redirect( add_query_arg( array( 'page' => 'barabi_core_dashboard' ), esc_url( admin_url( 'admin.php' ) ) ) );
			}
		}
		...
		function purchase_code_registration() {
			...
			if ( ! isset( $_POST ) || empty( $_POST ) ) {
				return esc_html__( 'All fields are empty', 'barabi-core' );
			} else {
		...
		function register_purchase_code() {
			...
			if ( empty( $data['purchase_code'] ) || empty( $data['email'] ) ) {
				qode_framework_get_ajax_status(
					'error',
					esc_html__( 'Purchase Code and Email are empty', 'barabi-core' ),
					array(
						'purchase_code' => false,
						'email'         => false,
					)
				...
				qode_framework_get_ajax_status( 'error', esc_html__( 'Purchase Code is empty', 'barabi-core' ), array( 'purchase_code' => false ) );
			} elseif ( empty( $data['email'] ) ) {
				qode_framework_get_ajax_status( 'error', esc_html__( 'Email is empty', 'barabi-core' ), array( 'email' => false ) );
			}
			...
			$url = add_query_arg(
				array(
					...
					'email'         => $data['email'],
					'profile'       => BARABI_CORE_PROFILE_SLUG . '-themes',
		...
		function api_connection( $url ) {
			...
			if ( 200 !== intval( $response_code ) ) {
				return new WP_Error( 'bad_request', esc_html__( 'Bad request', 'barabi-core' ) );
			}

			$json = json_decode( wp_remote_retrieve_body( $response ), true );

			if ( empty( $json ) || ! is_array( $json ) ) {
				return new WP_Error( 'invalid_response', esc_html__( 'Invalid Response', 'barabi-core' ) );
			}
		...
		function response_codes( $code, $data = array() ) {
			...
			switch ( $code ) :
				case 200:
					$message = esc_html__( 'Failed to validate code due to an error', 'barabi-core' );
					break;
				case 400:
					$message = esc_html__( 'Parameter or argument in the request was invalid', 'barabi-core' );
					break;
				case 401:
					$message = esc_html__( 'The authorization header is missing. Verify that your code is correct.', 'barabi-core' );
					break;
				case 403:
					$message = esc_html__( 'Personal token is incorrect or does not have the required permission(s)', 'barabi-core' );
					break;
				case 404:
					$message = esc_html__( 'The purchase code is invalid', 'barabi-core' );
					break;
				case 601:
					$message = esc_html__( 'You successfully activated theme', 'barabi-core' );
					break;
				case 602:
					$message = esc_html__( 'Code is valid', 'barabi-core' );
					break;
				case 603:
					$message = esc_html__( 'You successfully added demo', 'barabi-core' );
					break;
				case 604:
					$message = esc_html__( 'You successfully deregister theme', 'barabi-core' );
					break;
				case 650:
					$registered_url = '';
					...
					$message = sprintf(
						esc_html__( 'This code was already used to register another domain%s. Please deregister your code there so that you can use it for registering here.', 'barabi-core' ),
						$registered_url
					);
					break;
				case 651:
					$message = esc_html__( 'Error occurred during activation', 'barabi-core' );
					break;
				case 652:
					$message = esc_html__( 'Code is invalid', 'barabi-core' );
					break;
				case 653:
					$message = esc_html__( 'Error occurred during adding', 'barabi-core' );
					break;
				case 654:
					$message = esc_html__( 'Error occurred during deactivation', 'barabi-core' );
					break;
			endswitch;
	...
	BarabiCore_Dashboard::get_instance();
}
