<div class="qodef-reviews-list-info qodef-reviews-per-mark">
	...
	if ( isset( $with_title ) && $with_title ) { ?>
			<h3 class="qodef-reviews-title"><?php esc_html_e( 'Reviews', 'barabi-core' ); ?></h3>
		<?php
	}
	?>
	<?php foreach ( $post_ratings as $rating ) { ?>
		<?php
		$average_rating     = barabi_core_post_average_rating( $rating );
		$rating_count       = $rating['count'];
		$rating_count_label = 1 === $rating_count ? esc_html__( 'Rating', 'barabi-core' ) : esc_html__( 'Ratings', 'barabi-core' );
		...
		<div class="qodef-reviews-number-holder">
			<div class="qodef-reviews-number-wrapper">
				...
				<span class="qodef-stars-wrapper">
						<span class="qodef-review-rating">
							<?php echo barabi_core_reviews_get_rating_html( '', $average_rating, 0 ); ?>
						</span>
						<span class="qodef-reviews-count">
							<?php echo esc_html__( 'Rated', 'barabi-core' ) . ' ' . $average_rating . ' ' . esc_html__( 'out of', 'barabi-core' ) . ' ' . $rating_count . ' ' . $rating_count_label; ?>
						</span>
					</span>
			</div>
			<div class="qodef-rating-percentage-wrapper">
				...
				foreach ( $rating_marks as $item => $value ) {
					...
					echo BarabiCore_Progress_Bar_Shortcode::call_shortcode( $pb_params );
				}
				?>
			</div>
		</div>
