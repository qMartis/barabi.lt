...
if ( ! function_exists( 'barabi_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function barabi_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'BarabiCore_Image_Gallery_Shortcode';

		return $shortcodes;
	}

	add_filter( 'barabi_core_filter_register_shortcodes', 'barabi_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'BarabiCore_List_Shortcode' ) ) {
	class BarabiCore_Image_Gallery_Shortcode extends BarabiCore_List_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( BARABI_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'barabi_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'barabi-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'barabi-core' ) );
			$this->set_scripts(
				array(
			...
			$this->set_option(
				array(
					...
					'name'       => 'images',
					'multiple'   => 'yes',
					'title'      => esc_html__( 'Images', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'zoom_on_hover',
					'title'         => esc_html__( 'Zoom on Hover', 'barabi-core' ),
			...
			$this->map_list_options(
				array(
					...
					'group'                 => esc_html__( 'Gallery Settings', 'barabi-core' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'barabi_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return barabi_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
