...
abstract class BarabiCore_List_Shortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'barabi_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );
	...
	public function map_list_options( $params = array() ) {
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behavior', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => barabi_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => array(
						''      => esc_html__( 'Original', 'barabi-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => barabi_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
					'default_value' => '3',
					'group'         => $group,
					'dependency'    => array(
						'hide' => array(
		...
		$this->set_option(
			array(
				...
				'title'         => esc_html__( 'Custom Items Vertical Spacing - 680', 'barabi-core' ),
				'default_value' => '',
				'dependency'    => array(
					'show' => array(
						'vertical_space' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Row Height', 'barabi-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Max Row Height', 'barabi-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Last Row Treshold', 'barabi-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option(
			array(
				...
				'title'         => esc_html__( 'Choose Slide Animation', 'barabi-core' ),
				'options'       => array(
					''     => esc_html__( 'Default (Slide)', 'barabi-core' ),
					'fade' => esc_html__( 'Fade', 'barabi-core' ),
				),
				'default_value' => '',
				'description'   => esc_html__( 'Number of columns will be set to one.', 'barabi-core' ),
				'dependency'    => $dependency,
				'group'         => $group,
			)
		);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_direction', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Choose Slide Direction', 'barabi-core' ),
					'options'       => array(
						''         => esc_html__( 'Default (Horizontal)', 'barabi-core' ),
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_hidden_slides', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Show Hidden Slides', 'barabi-core' ),
					'options'     => barabi_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'Works best with horizontal slide direction and slide animation.', 'barabi-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Loop Additional Slides', 'barabi-core' ),
					'description' => esc_html__( 'Default value is 0 and this option works only when Show Hidden Slides option is enabled', 'barabi-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_centered_slides', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Enable Centered Slides', 'barabi-core' ),
					'options'     => barabi_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'If true, then active slide will be centered, not always on the left side.', 'barabi-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_mousewheel_navigation', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Enable Slider Mousewheel Navigation', 'barabi-core' ),
					'options'     => barabi_core_get_select_type_options_pool( 'yes_no' ),
					'description' => esc_html__( 'Enables navigation through slides using mouse wheel.', 'barabi-core' ),
					'dependency'  => $dependency,
					'group'       => $group,
				)
			);
		...
		if ( ! empty( $include_slider_option ) && in_array( 'slider_drag_cursor', $include_slider_option, true ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Show Drag Cursor', 'barabi-core' ),
					'options'    => barabi_core_get_select_type_options_pool( 'yes_no' ),
					'dependency' => $dependency,
					'group'      => $group,
				)
		...
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Show Slider Navigation', 'barabi-core' ),
				'options'    => barabi_core_get_select_type_options_pool( 'yes_no', true, '', array( 'combo' => esc_html__( 'Combo', 'barabi-core' ) ) ),
				'dependency' => $dependency,
				'group'      => $group,
			)
		);
		...
		$this->set_option(
			array(
				...
				'title'       => esc_html__( 'Show Slider Pagination', 'barabi-core' ),
				'options'     => barabi_core_get_select_type_options_pool( 'yes_no' ),
				'description' => esc_html__( 'This option does\'t work when Show Slider Navigation option is set to Combo', 'barabi-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
	...
	public function get_list_styles( $atts ) {
		$styles = barabi_core_get_gutter_custom_styles( 'space_', 'vertical_space_', $atts, true );
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && 'masonry' === $atts['behavior'] && ! empty( $atts['masonry_images_proportion'] ) && 'fixed' === $atts['masonry_images_proportion'] ) {
			...
			$image_dimension              = barabi_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}

		if ( ! empty( $atts['behavior'] ) && in_array( $atts['behavior'], array( 'columns', 'slider' ), true ) ) {
			$image_dimension = array(
				'size'  => $atts['images_proportion'],
				'class' => barabi_core_get_custom_image_size_class_name( $atts['images_proportion'] ),
			);
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		if ( isset( $atts['space'] ) ) {
			if ( 'custom' === $atts['space'] ) {
				...
				$data['spaceBetween'] = barabi_core_get_space_value( $atts['space'] );
			}
		} else {
			$data['spaceBetween'] = 0;
		}
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'barabi-core' );
		...
		$this->set_option(
			array(
				'field_type'    => 'text',
				'name'          => 'posts_per_page',
				'title'         => esc_html__( 'Posts per Page', 'barabi-core' ),
				'default_value' => '9',
				'group'         => $group,
			)
		);
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'orderby', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'orderby',
					'title'         => esc_html__( 'Order By', 'barabi-core' ),
					'options'       => barabi_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
					'default_value' => 'date',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'order', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'order',
					'title'         => esc_html__( 'Order', 'barabi-core' ),
					'options'       => barabi_core_get_select_type_options_pool( 'order' ),
					'default_value' => 'DESC',
					'group'         => $group,
				)
			);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option, true ) ) ) {
			...
			do_action( 'barabi_core_action_map_query_options_before_additional', $group );

			$additional_params = apply_filters(
				'barabi_core_filter_map_additional_query_params',
				array(
					''       => esc_html__( 'No', 'barabi-core' ),
					'id'     => esc_html__( 'Post IDs', 'barabi-core' ),
					'tax'    => esc_html__( 'Taxonomy Slug', 'barabi-core' ),
					'author' => esc_html__( 'Author Name', 'barabi-core' ),
				),
				$this->get_base()
			);
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Additional Params', 'barabi-core' ),
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'post_ids',
					'title'       => esc_html__( 'Posts IDs', 'barabi-core' ),
					'description' => esc_html__( 'Separate post IDs with commas', 'barabi-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				$this->set_option(
					array(
						'field_type' => 'select',
						'name'       => 'tax',
						'title'      => esc_html__( 'Taxonomy', 'barabi-core' ),
				...
				$this->set_option(
					array(
						'field_type' => 'text',
						'name'       => 'tax_slug',
						'title'      => esc_html__( 'Taxonomy Slug', 'barabi-core' ),
						'group'      => $group,
						'dependency' => array(
							'show' => array(
								'additional_params' => array(
				...
				$this->set_option(
					array(
						'field_type'  => 'text',
						'name'        => 'tax__in',
						'title'       => esc_html__( 'Taxonomy IDs', 'barabi-core' ),
						'description' => esc_html__( 'Separate taxonomy IDs with commas', 'barabi-core' ),
						'group'       => $group,
						'dependency'  => array(
							'show' => array(
								'additional_params' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'author_slug',
					'title'      => esc_html__( 'Author Slug', 'barabi-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			do_action( 'barabi_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'barabi_core_filter_additional_query_args', $args, $atts, $this->get_post_type() );
	...
	public function map_layout_options( $params = array() ) {
		...
		$this->set_option(
			array(
				...
				'options'       => $layouts,
				'default_value' => apply_filters( 'barabi_core_filter_map_layout_options_default_value', $default_value, $this->get_base(), $params ),
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'barabi-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'enable_filter',
					'title'         => esc_html__( 'Enable Filter', 'barabi-core' ),
					'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'barabi-core' ),
			...
			do_action( 'barabi_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Pagination Top Margin (px or %)', 'barabi-core' ),
					'default_value' => '',
					'group'         => $group_name,
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'barabi_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function load_assets() {
		do_action( 'barabi_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
