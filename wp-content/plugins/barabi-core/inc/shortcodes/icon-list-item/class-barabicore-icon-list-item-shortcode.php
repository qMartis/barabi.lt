...
if ( ! function_exists( 'barabi_core_add_icon_list_item_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function barabi_core_add_icon_list_item_shortcode( $shortcodes ) {
		$shortcodes[] = 'BarabiCore_Icon_List_Item_Shortcode';

		return $shortcodes;
	}

	add_filter( 'barabi_core_filter_register_shortcodes', 'barabi_core_add_icon_list_item_shortcode' );
}

if ( class_exists( 'BarabiCore_Shortcode' ) ) {
	class BarabiCore_Icon_List_Item_Shortcode extends BarabiCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( BARABI_CORE_SHORTCODES_URL_PATH . '/icon-list-item' );
			$this->set_base( 'barabi_core_icon_list_item' );
			$this->set_name( esc_html__( 'Icon List Item', 'barabi-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon list item element', 'barabi-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'item_margin_bottom',
					'title'      => esc_html__( 'Item Margin Bottom', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'barabi_core_icon_list_item', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return barabi_core_get_template_part( 'shortcodes/icon-list-item', 'templates/icon-list-item', '', $atts );
		}
