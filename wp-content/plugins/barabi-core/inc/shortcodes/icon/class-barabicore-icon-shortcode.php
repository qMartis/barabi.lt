...
if ( ! function_exists( 'barabi_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function barabi_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'BarabiCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'barabi_core_filter_register_shortcodes', 'barabi_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'BarabiCore_Shortcode' ) ) {
	class BarabiCore_Icon_Shortcode extends BarabiCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( BARABI_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'barabi_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'barabi-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'barabi-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type' => 'iconpack',
					'name'       => 'main_icon',
					'title'      => esc_html__( 'Icon', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'barabi-core' ),
						'small'   => esc_html__( 'Small', 'barabi-core' ),
						'medium'  => esc_html__( 'Medium', 'barabi-core' ),
						'large'   => esc_html__( 'Large', 'barabi-core' ),
						'huge'    => esc_html__( 'Huge', 'barabi-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'barabi-core' ),
					'group'      => esc_html__( 'Style', 'barabi-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'barabi-core' ),
					'group'      => esc_html__( 'Style', 'barabi-core' ),
				)
			);
			$this->set_option(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'link',
					'title'      => esc_html__( 'Icon Link', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'barabi_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return barabi_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
