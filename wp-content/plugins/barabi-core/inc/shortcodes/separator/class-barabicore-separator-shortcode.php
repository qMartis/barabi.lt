...
if ( ! function_exists( 'barabi_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function barabi_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'BarabiCore_Separator_Shortcode';

		return $shortcodes;
	}

	add_filter( 'barabi_core_filter_register_shortcodes', 'barabi_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'BarabiCore_Shortcode' ) ) {
	class BarabiCore_Separator_Shortcode extends BarabiCore_Shortcode {
		var $breakpoints = array( 1600, 1440, 1366, 1280, 1024, 768, 680, 480 ); // match array in scss file: _separator-default.scss

		public function map_shortcode() {
			$this->set_shortcode_path( BARABI_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'barabi_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'barabi-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'barabi-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option( array(
					...
					'title'      => esc_html__( 'Show Separator', 'barabi-core' ),
				)
			);
			$breakpoints = $this->breakpoints;
			foreach ( $breakpoints as $breakpoint ) {
				$title_label = sprintf( '%s ' . $breakpoint . ' %s', esc_html__( 'Show Separator Below', 'barabi-core' ), esc_html__( 'px', 'barabi-core' ) );
				$this->set_option( array(
						'field_type' => 'select',
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'width',
					'title'      => esc_html__( 'Width (px or %)', 'barabi-core' ),
					'group'      => esc_html__( 'Style', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'thickness',
					'title'      => esc_html__( 'Thickness (px)', 'barabi-core' ),
					'group'      => esc_html__( 'Style', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_top',
					'title'      => esc_html__( 'Margin Top (px or %)', 'barabi-core' ),
					'group'      => esc_html__( 'Style', 'barabi-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_bottom',
					'title'      => esc_html__( 'Margin Bottom (px or %)', 'barabi-core' ),
					'group'      => esc_html__( 'Style', 'barabi-core' ),
				)
			);
		}
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'barabi_core_separator', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return barabi_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
