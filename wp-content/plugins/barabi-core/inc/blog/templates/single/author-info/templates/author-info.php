<?php
$is_enabled = barabi_core_get_post_value_through_levels( 'qodef_blog_single_enable_author_info' );

if ( 'yes' === $is_enabled && '' !== get_the_author_meta( 'description' ) ) {
	$author_id       = get_the_author_meta( 'ID' );
	$author_position = get_the_author_meta( 'qodef_user_position' );
	$author_link     = get_author_posts_url( $author_id );
	$email_enabled   = 'yes' === barabi_core_get_post_value_through_levels( 'qodef_blog_single_enable_author_info_email' );
	$user_socials    = barabi_core_get_author_social_networks( $author_id );
	?>
					...
					<div class="qodef-m-social-icons">
						<?php foreach ( $user_socials as $social ) { ?>
							<a itemprop="url" class="<?php echo esc_attr( $social['class'] ); ?>" href="<?php echo esc_url( $social['url'] ); ?>" target="_blank">
								<?php echo barabi_core_get_svg_icon( $social['network'] ); ?>
							</a>
						<?php } ?>
					</div>
				<?php } ?>
