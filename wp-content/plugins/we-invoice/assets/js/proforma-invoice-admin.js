/**
 * Proforma Invoice Admin JavaScript
 */
(function($) {
    'use strict';
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.we-generate-invoice', function(e) {
            ...
            $button.prop('disabled', true).text(weProformaInvoice.strings.generating);
            
            $.ajax({
                url: weProformaInvoice.ajax_url,
                type: 'POST',
                data: {
                    action: 'generate_proforma_invoice',
                    order_id: orderId,
                    nonce: weProformaInvoice.nonce
                },
                success: function(response) {
                    if (response.success) {
                        showNotice(weProformaInvoice.strings.success, 'success');
                        location.reload(); // Reload to show the generated invoice
                    } else {
                        showNotice(response.data.message || weProformaInvoice.strings.error, 'error');
                    }
                },
                error: function() {
                    showNotice(weProformaInvoice.strings.error, 'error');
                },
                complete: function() {
                    $button.prop('disabled', false).html('<span class="dashicons dashicons-plus-alt"></span> Generate Proforma Invoice');
                }
            });
        });
        ...
        $(document).on('click', '.we-send-invoice', function(e) {
            ...
            $button.prop('disabled', true).text(weProformaInvoice.strings.sending);
            
            $.ajax({
                url: weProformaInvoice.ajax_url,
                type: 'POST',
                data: {
                    action: 'send_proforma_invoice',
                    order_id: orderId,
