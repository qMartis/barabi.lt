...
if ( ! function_exists( 'teenglow_core_add_wishlist_profile_navigation_item' ) ) {
	function teenglow_core_add_wishlist_profile_navigation_item( $items, $dashboard_url ) {
		$items['my-wishlist'] = array(
			'url'         => esc_url( add_query_arg( array( 'user-action' => 'my-wishlist' ), $dashboard_url ) ),
			'text'        => esc_html__( 'My Wishlist', 'teenglow-core' ),
	...
	add_filter( 'teenglow_membership_filter_dashboard_navigation_action_pages', 'teenglow_core_add_wishlist_profile_navigation_item', 10, 2 );
}

if ( ! function_exists( 'teenglow_core_add_wishlist_profile_navigation_pages' ) ) {
	function teenglow_core_add_wishlist_profile_navigation_pages( $html, $action ) {

		if ( 'my-wishlist' === $action ) {
			$atts = array();

			$html = teenglow_core_get_template_part( 'wishlist', '/profile/templates/my-wishlist', '', $atts );
		}

		return $html;
	}

	add_filter( 'teenglow_membership_filter_dashboard_page', 'teenglow_core_add_wishlist_profile_navigation_pages', 10, 2 );
}
