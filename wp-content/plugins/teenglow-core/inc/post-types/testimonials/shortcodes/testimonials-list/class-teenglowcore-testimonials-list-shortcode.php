...
if ( ! function_exists( 'teenglow_core_add_testimonials_list_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function teenglow_core_add_testimonials_list_shortcode( $shortcodes ) {
		$shortcodes[] = 'TeenglowCore_Testimonials_List_Shortcode';

		return $shortcodes;
	}

	add_filter( 'teenglow_core_filter_register_shortcodes', 'teenglow_core_add_testimonials_list_shortcode' );
}

if ( class_exists( 'TeenglowCore_List_Shortcode' ) ) {
	class TeenglowCore_Testimonials_List_Shortcode extends TeenglowCore_List_Shortcode {

		public function __construct() {
			$this->set_post_type( 'testimonials' );
			$this->set_post_type_additional_taxonomies( array( 'testimonials-category' ) );
			$this->set_layouts( apply_filters( 'teenglow_core_filter_testimonials_list_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'teenglow_core_filter_testimonials_list_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( TEENGLOW_CORE_CPT_URL_PATH . '/testimonials/shortcodes/testimonials-list' );
			$this->set_base( 'teenglow_core_testimonials_list' );
			$this->set_name( esc_html__( 'Testimonials List', 'teenglow-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays list of testimonials', 'teenglow-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Center Slider', 'teenglow-core' ),
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Show Average Rating Information', 'teenglow-core' ),
					'options'       => teenglow_core_get_select_type_options_pool( 'yes_no', false ),
					'default_value' => 'yes',
					'dependency'    => array(
						'show' => array(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'teenglow_core_testimonials_list', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return teenglow_core_get_template_part( 'post-types/testimonials/shortcodes/testimonials-list', 'templates/content', $atts['behavior'], $atts );
		}
