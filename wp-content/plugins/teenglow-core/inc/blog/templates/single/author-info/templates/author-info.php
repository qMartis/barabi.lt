<?php
$is_enabled = teenglow_core_get_post_value_through_levels( 'qodef_blog_single_enable_author_info' );

if ( 'yes' === $is_enabled && '' !== get_the_author_meta( 'description' ) ) {
	$author_id       = get_the_author_meta( 'ID' );
	$author_position = get_the_author_meta( 'qodef_user_position' );
	$author_link     = get_author_posts_url( $author_id );
	$email_enabled   = 'yes' === teenglow_core_get_post_value_through_levels( 'qodef_blog_single_enable_author_info_email' );
	$user_socials    = teenglow_core_get_author_social_networks( $author_id );
	?>
	<div id="qodef-author-info" class="qodef-m">
		<div class="qodef-m-inner">
			<div class="qodef-m-image">
				<a itemprop="url" href="<?php echo esc_url( $author_link ); ?>">
					<?php echo get_avatar( $author_id, 166 ); ?>
				</a>
			</div>
			<div class="qodef-m-content">
				...
				<?php if ( ! empty( $author_position ) ) ?>
					<span class="qodef-author-position"><?php echo esc_html( $author_position ); ?></span>
				<?php ?>
					...
					<div class="qodef-m-social-icons">
						<?php foreach ( $user_socials as $social ) { ?>
							<a itemprop="url" class="<?php echo esc_attr( $social['class'] ); ?>" href="<?php echo esc_url( $social['url'] ); ?>" target="_blank">
								<?php echo teenglow_core_get_svg_icon( $social['network'] ); ?>
							</a>
						<?php } ?>
					</div>
				<?php } ?>
