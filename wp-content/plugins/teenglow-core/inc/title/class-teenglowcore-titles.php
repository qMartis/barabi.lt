...
class TeenglowCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'teenglow_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'teenglow_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return TeenglowCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( TEENGLOW_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( TEENGLOW_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = teenglow_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'teenglow_core_filter_register_title_layouts', array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'teenglow_filter_title_template' : 'teenglow_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = teenglow_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = teenglow_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = teenglow_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$in_grid            = teenglow_core_get_post_value_through_levels( 'qodef_set_page_title_area_in_grid' );
		$image_params       = teenglow_core_get_page_title_image_params();
		...
		if ( ! empty( $in_grid ) ) {
			$classes[] = 'yes' === $in_grid ? 'qodef--in-grid' : 'qodef--full-width';
		}
	...
	function add_inline_styles( $style ) {
		...
		$height                   = teenglow_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = teenglow_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding            = apply_filters( 'teenglow_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'teenglow_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = teenglow_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = teenglow_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = teenglow_core_get_page_title_image_params();
		$title_additional_padding = teenglow_core_get_post_value_through_levels( 'qodef_page_title_additional_bottom_offset' );
		$title_side_padding       = teenglow_core_get_post_value_through_levels( 'qodef_page_title_side_padding' );
		$title_in_grid            = teenglow_core_get_post_value_through_levels( 'qodef_set_page_title_area_in_grid' );
		...
		if ( ! empty( $title_additional_padding ) ) {
			$styles['padding-bottom'] = intval( $title_additional_padding ) . 'px';
		}
		...
		if ( ! empty( $title_side_padding ) && 'no' === $title_in_grid ) {

			if ( qode_framework_string_ends_with_space_units( $title_side_padding ) ) {
				$styles['padding-left']  = $title_side_padding;
				$styles['padding-right'] = $title_side_padding;
			} else {
				$styles['padding-left']  = intval( $title_side_padding ) . 'px';
				$styles['padding-right'] = intval( $title_side_padding ) . 'px';
			}
		}
		...
		$title_color = teenglow_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
TeenglowCore_Titles::get_instance();
