...
if ( ! function_exists( 'teenglow_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function teenglow_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'TeenglowCore_Breadcrumbs_Title';

		return $layouts;
	}

	add_filter( 'teenglow_core_filter_register_title_layouts', 'teenglow_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'teenglow_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function teenglow_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'teenglow-core' );

		return $layouts;
	}

	add_filter( 'teenglow_core_filter_title_layout_options', 'teenglow_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'teenglow_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function teenglow_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters(
			'teenglow_core_filter_breadcrumbs_label',
			array(
				'home'        => esc_html__( 'Home', 'teenglow-core' ),
				'tag'         => esc_html__( 'Posts tagged "%s"', 'teenglow-core' ),
				'author'      => esc_html__( 'Posted by %s', 'teenglow-core' ),
				'search'      => esc_html__( 'Search results for "%s"', 'teenglow-core' ),
				'404'         => esc_html__( '404 - Page not found', 'teenglow-core' ),
				'query_paged' => esc_html__( '(Page %s)', 'teenglow-core' ),
			)
		);

		// Breadcrumbs variables
		$settings = apply_filters(
			'teenglow_core_filter_breadcrumbs_settings',
			array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'teenglow_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'teenglow_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}
