...
if ( ! function_exists( 'teenglow_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function teenglow_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'TeenglowCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'teenglow_core_filter_register_shortcodes', 'teenglow_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'TeenglowCore_Shortcode' ) ) {
	class TeenglowCore_Icon_Shortcode extends TeenglowCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TEENGLOW_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'teenglow_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'teenglow-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'teenglow-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type' => 'iconpack',
					'name'       => 'main_icon',
					'title'      => esc_html__( 'Icon', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'teenglow-core' ),
						'small'   => esc_html__( 'Small', 'teenglow-core' ),
						'medium'  => esc_html__( 'Medium', 'teenglow-core' ),
						'large'   => esc_html__( 'Large', 'teenglow-core' ),
						'huge'    => esc_html__( 'Huge', 'teenglow-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'teenglow-core' ),
					'group'      => esc_html__( 'Style', 'teenglow-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'teenglow-core' ),
					'group'      => esc_html__( 'Style', 'teenglow-core' ),
				)
			);
			$this->set_option(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'link',
					'title'      => esc_html__( 'Icon Link', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'teenglow_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return teenglow_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
