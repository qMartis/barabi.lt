...
if ( ! function_exists( 'teenglow_core_add_text_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function teenglow_core_add_text_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'TeenglowCore_Text_Marquee_Shortcode';

		return $shortcodes;
	}

	add_filter( 'teenglow_core_filter_register_shortcodes', 'teenglow_core_add_text_marquee_shortcode' );
}

if ( class_exists( 'TeenglowCore_Shortcode' ) ) {
	class TeenglowCore_Text_Marquee_Shortcode extends TeenglowCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'teenglow_core_filter_text_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'teenglow_core_filter_text_marquee_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( TEENGLOW_CORE_SHORTCODES_URL_PATH . '/text-marquee' );
			$this->set_base( 'teenglow_core_text_marquee' );
			$this->set_name( esc_html__( 'Text Marquee', 'teenglow-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Text Marquee element', 'teenglow-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'text_1',
					'title'      => esc_html__( 'Text 1', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'text_1_color',
					'title'      => esc_html__( 'Text 1 Color', 'teenglow-core' ),
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'text_2',
					'title'      => esc_html__( 'Text 2', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'text_2_color',
					'title'      => esc_html__( 'Text 2 Color', 'teenglow-core' ),
					'group'      => esc_html__( 'Style', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'text_3',
					'title'      => esc_html__( 'Text 3', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'text_3_color',
					'title'      => esc_html__( 'Text 3 Color', 'teenglow-core' ),
					'group'      => esc_html__( 'Style', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Enable Star Separator', 'teenglow-core' ),
					'default_value' => 'yes',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Star Separator Color', 'teenglow-core' ),
					'group'      => esc_html__( 'Style', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'direction',
					'title'         => esc_html__( 'Direction', 'teenglow-core' ),
					'options'       => array(
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'duration',
					'title'       => esc_html__( 'Duration (in seconds)', 'teenglow-core' ),
					'description' => esc_html__( 'Default value is 25s', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'letter_spacing',
					'title'      => esc_html__( 'Letter Spacing (px or em)', 'teenglow-core' ),
					'group'      => esc_html__( 'Typography Style', 'teenglow-core' ),
				)
			);
			$this->set_option(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_1440',
					'title'       => esc_html__( 'Font Size', 'teenglow-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 1440', 'teenglow-core' ),
					'group'       => esc_html__( 'Screen Size 1440 Style', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return teenglow_core_get_template_part( 'shortcodes/text-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
