...
if ( ! function_exists( 'teenglow_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function teenglow_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'TeenglowCore_Image_Gallery_Shortcode';

		return $shortcodes;
	}

	add_filter( 'teenglow_core_filter_register_shortcodes', 'teenglow_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'TeenglowCore_List_Shortcode' ) ) {
	class TeenglowCore_Image_Gallery_Shortcode extends TeenglowCore_List_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TEENGLOW_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'teenglow_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'teenglow-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'teenglow-core' ) );
			$this->set_scripts(
				array(
			...
			$this->set_option(
				array(
					...
					'name'       => 'images',
					'multiple'   => 'yes',
					'title'      => esc_html__( 'Images', 'teenglow-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'zoom_on_hover',
					'title'         => esc_html__( 'Zoom on Hover', 'teenglow-core' ),
			...
			$this->map_list_options(
				array(
					...
					'group'                 => esc_html__( 'Gallery Settings', 'teenglow-core' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'teenglow_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return teenglow_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty( $atts['image_shadow'] ) && 'yes' === $atts['image_shadow'] ? 'qodef-shadow--enabled' : '';
			$holder_classes[] = ! empty( $atts['zoom_on_hover'] ) && 'yes' === $atts['zoom_on_hover'] ? 'qodef--zoom' : '';
