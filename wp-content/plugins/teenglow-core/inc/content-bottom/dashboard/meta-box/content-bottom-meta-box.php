<?php

if ( ! function_exists( 'teenglow_core_add_content_bottom_meta_box' ) ) {
	/**
	 * Function that add general meta box options for this module
	 *
	 * @param object $page
	 */
	function teenglow_core_add_content_bottom_meta_box( $page ) {

		if ( $page ) {

			$content_bottom_tab = $page->add_tab_element(
				array(
					'name'        => 'tab-content-bottom',
					'icon'        => 'fa fa-cog',
					'title'       => esc_html__( 'Content Bottom Settings', 'teenglow-core' ),
					'description' => esc_html__( 'Content bottom layout settings', 'teenglow-core' ),
				)
			);

			$content_bottom_tab->add_field_element(
				array(
					'field_type'  => 'select',
					'name'        => 'qodef_enable_content_bottom',
					'title'       => esc_html__( 'Enable Content Bottom', 'teenglow-core' ),
					'description' => esc_html__( 'Use this option to enable/disable page content bottom', 'teenglow-core' ),
					'options'     => teenglow_core_get_select_type_options_pool( 'no_yes' ),
				)
			);

			$content_bottom_section = $content_bottom_tab->add_section_element(
				array(
					'name'       => 'qodef_content_bottom_section',
					'title'      => esc_html__( 'Content Bottom Area', 'teenglow-core' ),
					'dependency' => array(
						'hide' => array(
							'qodef_enable_content_bottom' => array(
								'values'        => 'no',
								'default_value' => '',
							),
						),
					),
				)
			);
		}

		$custom_sidebars = teenglow_core_get_custom_sidebars();

		if ( ! empty( $custom_sidebars ) && count( $custom_sidebars ) > 1 ) {
			$content_bottom_section->add_field_element(
				array(
					'field_type'  => 'select',
					'name'        => 'qodef_content_bottom_custom_sidebar',
					'title'       => esc_html__( 'Custom Widget Area', 'teenglow-core' ),
					'description' => esc_html__( 'Choose a custom widget area to display in content bottom area', 'teenglow-core' ),
					'options'     => $custom_sidebars,
				)
			);
		}

		$content_bottom_section->add_field_element(
			array(
				'field_type'  => 'select',
				'name'        => 'qodef_content_bottom_in_grid',
				'title'       => esc_html__( 'Content Bottom In Grid', 'teenglow-core' ),
				'description' => esc_html__( 'Enabling this option will set content bottom area to be in grid', 'teenglow-core' ),
				'options'     => teenglow_core_get_select_type_options_pool( 'no_yes' ),
			)
		);

		$content_bottom_section->add_field_element(
			array(
				'field_type'  => 'color',
				'name'        => 'qodef_content_bottom_background_color',
				'title'       => esc_html__( 'Background Color', 'teenglow-core' ),
				'description' => esc_html__( 'Enter content bottom background color', 'teenglow-core' ),
			)
		);
	}

	add_action( 'teenglow_core_action_after_general_meta_box_map', 'teenglow_core_add_content_bottom_meta_box' );
}
